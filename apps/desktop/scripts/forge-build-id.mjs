...
const BUILD_ID_PATTERN =
  /^unsigned-[A-Za-z0-9](?:[A-Za-z0-9_-]{0,84}[A-Za-z0-9])?$/u;
...
export function createForgeBuildIdentifier(now = new Date(), pid = process.pid) {
  const timestamp = now.toISOString().replace(/[:.]/gu, "-");
  return validateForgeBuildIdentifier(`unsigned-${timestamp}-${pid}`);
}
